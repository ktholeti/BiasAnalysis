    ...
    if model_id == "bertbase" or model_id == "bertlarge":  # "bert-base-uncased"
        ...
        if load_model:
            ...
            model = BertForSequenceClassification.from_pretrained(model_name,
                                                                  config=configuration)  # , num_labels=2)#
...
def compute_metrics(p):
    ...
    print("{} : compute_metrics - accuracy: {}; precision: {}; recall: {}; f1: {}".format(timestamp(True), accuracy,
                                                                                          precision, recall, f1))
    return {"accuracy": accuracy, "precision": precision, "recall": recall, "f1": f1}


def train(task, model_id, spec, tokenizer=None, model=None, eval_steps_=500, per_device_train_batch_size_=32,
          per_device_eval_batch_size_=32, num_train_epochs_=100):
    '''
    todo
    '''
    ...
    print(
        '{}: params: spec= {}; eval_steps_={}; per_device_train_batch_size_={}; per_device_eval_batch_size_={}; num_train_epochs_={}'.format(
            __name__, spec, eval_steps_, per_device_train_batch_size_, per_device_eval_batch_size_, num_train_epochs_))
...
def calc_acc(spec, tokenizer, model_id, task="foo", restricted_test_set=False):
    ...
    print(model_path)
    model = load_hf(model_id,
                    path_to_model=model_path)  # DistilBertForSequenceClassification.from_pretrained(, num_labels=2)
